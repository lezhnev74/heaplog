...
`
	...
	tests := []test{
		...
		{"!wrong", true},
		{"Report", true},
		{"report", true}, // case-sensitive matching enabled
		{"~Report", true},
		{"~(Report)", true},
		{"~Re.{3}t", true},
		{"!Report", false},
...
func TestClone(t *testing.T) {
	t.Run(
		"expr can be cloned", func(t *testing.T) {
...
func TestHashing(t *testing.T) {
	...
	t.Run(
		"hashes differ", func(t *testing.T) {
			q1 := "error"
			q2 := "~error"
	...
	t.Run(
		"expr is sorted before hashing", func(t *testing.T) {
			expr1 := &Expression{AND, []any{
				"OP1",
				&Expression{OR, []any{
					"OP3",
